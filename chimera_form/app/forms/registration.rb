class Registration
  ...
  def company_params
    sliced_params = params.slice(:company_name)
    sliced_params.each { |k| }.map { |key, value| { key.to_s.sub('company_', '').to_sym => value } }.inject
  end

  def company_address_params
    address_sliced_params = params.slice(:company_street, :company_city, :company_zip_code, :company_country)
    address_sliced_params.each { |k| }.map { |key, value| { key.to_s.sub('company_', '').to_sym => value } }.inject(:merge)
  end
